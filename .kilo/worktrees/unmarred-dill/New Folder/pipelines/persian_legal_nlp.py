...
def sent_tokenize(text: str, use_parsivar: bool = True) -> List[str]:
    ...
    if use_parsivar and _HAS_PARSIVAR:
        try:
            ...
            logger.warning(f"parsivar sentence tokenization failed: {e}, using fallback")
...
LEGAL_PATTERNS = {
    ...
    "ماده": r'ماده\s+(?:یک|دو|سه|چهار|پنج|شش|هفت|هشت|نه|ده|یازده|دوازده|سیزده|چهارده|پانزده|شانزده|هفده|هجده|نوزده|بیست|\d+)',
    "تبصره": r'تبصره\s+(?:\d+|یک|دو|سه|چهار|پنج|الحاقی)',
    "بند": r'بند\s+(?:\d+|[آ-ی])',
