...
class StateMachine:
    ...
    def transition(
        ...
        with self.state_lock:
            ...
            # Check conditions
            if not force:
                conditions_met, reason = self._check_conditions(trigger)
                if not conditions_met:
                    logger.warning(f"Transition conditions not met: {reason}")
                    self.failed_transitions += 1
                    return False, reason
    ...
    def _check_conditions(self, trigger: TransitionTrigger) -> tuple[bool, Optional[str]]:
        """Check all conditions for transition"""
        key = (self.current_state, trigger)
        conditions = self.transition_conditions.get(key, [])
        
        for condition in conditions:
            met, reason = condition.evaluate()
            if not met and condition.required:
                return False, reason
    ...
    def add_condition(
        ...
        condition = StateCondition(name, check, description, required)
