...
PADDLE_BINARY_PATH = "/home/haji/Desktop/Mahoun/venv/lib/python3.12/site-packages/paddle/libs"
PADDLE_ENVIRONMENT_PATH = "/home/haji/Desktop/Z1/Platform/venv/lib/python3.12/site-packages/paddle"
LEGACY_LOGIC_PATH = "/home/haji/Desktop/Z1/Platform/mahoun/pipelines/ingestion"
...
class TruthTraceSegment:
    ...
    table_json_structure: Optional[str] = None  # Deterministic Markdown representation of table
...
class HardenedPaddleOCR:
    ...
    def _create_truth_trace_segment(
        ...
        page_number: int,
        is_handwritten: bool = False,
        table_json_structure: Optional[str] = None,
    ...
    def _load_latest_checkpoint(self, document_id: str) -> Optional[OCRCheckpoint]:
        """
    ...
    def _pre_scan_for_complex_structures(self, image_path: Union[str, Path]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if any(keyword in filename for keyword in ['handwritten', 'hwr', 'marginalia', 'هامش', '파라프', 'table', '표', 'form', '양식']):
    ...
    def _process_with_paddlex(self, image_path: Union[str, Path]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                'table_markdowns': all_table_markdowns
            }
            ...
            try:
                ...
                segment = {
                    'text': combined_text,
                    'bounding_box': [[0,0],[0,0],[0,0],[0,0]],  # We don't have a bbox for the whole image from base OCR easily
                    'segment_type': 'text',
                    'is_handwritten': False,
                    'table_markdown': None,
                    'semantic_weight': 1.0
            ...
            if use_paddlex:
                ...
                if not paddlex_result or not paddlex_result.get('text', '').strip():
                    ...
                    for segment in segments:
                        ...
                        seg_segment_type = segment.get('segment_type', 'text')
                    ...
                    combined_text_for_check = combined_text  # We already have this
                    ...
                    if enable_checkpointing and document_id:
                        checkpoint = OCRCheckpoint(
                            ...
                            merkle_leaves=[merkle_leaf] + table_merkle_leaves,  # Store the text leaf and all table leaves
                ...
                if line and len(line) >= 2:
                    ...
                    if text and isinstance(text, str):
                        ...
                        if normalized_text:  # Only process non-empty text
                            ...
                            # Create truth trace segment
                            segment = self._create_truth_trace_segment(
                                normalized_text,
                                bbox,
                                float(conf),
                                page_number
