...
class AdaptiveTTLManager:
    ...
    def compute_ttl(self, query: str, entry: Optional[CacheEntry] = None) -> int:
        ...
        if stats["access_count"] > 0:
            ...
            stats["avg_interval"] = 0.7 * stats["avg_interval"] + 0.3 * interval
...
class SmartCache:
    ...
    def get_popular_queries(self, top_k: int = 10) -> List[Tuple[str, float]]:
        ...
        queries = [
            (entry.query, entry.popularity_score) for entry in self.l1_cache.values()
        ]
...
def main():
    ...
    for i, query in enumerate(queries, 1):
        ...
        if results is None:
            ...
            print("  → MISS, cached with adaptive TTL")
        else:
