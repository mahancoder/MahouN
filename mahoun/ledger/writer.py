...
class EvidenceLedgerWriter:
    ...
    def __init__(
        ...
        blockchain: Optional[ImmutableLedger] = None,
    ...
    def create_blockchain(
        ...
        """
        blockchain = ImmutableLedger(storage_path)
    ...
    def write(self, entry: LedgerEntry) -> str:
        """
        Write entry to ledger with cryptographic integrity.
        ...
        Raises:
            RuntimeError: If write fails (blocks verdict publication per EL-I3)
            ValueError: If validation fails
        """
        try:
            # Validate entry with deep validation if graph_builder available
            ...
            if self.blockchain:
                ...
                logger.info(f"Ledger entry written to blockchain: verdict={entry.verdict_id}, block={block.index}, hash={block.hash[:16]}...")
                return block.hash
            else:
