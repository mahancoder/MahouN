...
class ImmutableLedger:
    ...
    - Process-safe via filelock.FileLock
    """
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        if storage_path:
            self._file_lock = FileLock(f"{storage_path}.lock")
    ...
    def append(self, entry: LedgerEntry) -> Block:
        ...
        with self._local_lock:
            # If storage_path is provided, use file lock
            if self._file_lock:
    ...
    def _append_logic(self, entry: LedgerEntry) -> Block:
        """Internal append logic after locks are acquired"""
        # Re-load from disk to ensure we have latest chain in multi-process environments
        ...
        log.debug(
            f"Appended block {new_block.index}: verdict={entry.verdict_id[:8]}..."
        )
    ...
    def export_audit_report(
        ...
        report = {
            ...
            "entries": [self._entry_to_export_dict(entry) for entry in entries],
        }
    ...
    def _entry_to_export_dict(self, entry: LedgerEntry) -> dict:
        """
        Convert LedgerEntry (dataclass) to dict for export.
        
        HARDENING PATCH P05: LedgerEntry is a frozen dataclass, not Pydantic.
        HARDENING PATCH P11: Canonical serialization.
        """
        from mahoun.ledger.models import canonical_serialize
        return canonical_serialize(entry)
